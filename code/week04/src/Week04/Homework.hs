...
payContract = do
  ...
  Contract.handleError (\err -> Contract.logError $ "caught: " ++ unpack err)
    $ void
    $ submitTx tx
  payContract
...
payTrace amount1 amount2 = do
  ...
  let recipient = mockWalletPaymentPubKeyHash $ knownWallet 2
  callEndpoint @"pay" h1
    $ PayParams { ppRecipient = recipient, ppLovelace = amount1 }
  void $ Emulator.waitNSlots 1
  callEndpoint @"pay" h1
    $ PayParams { ppRecipient = recipient, ppLovelace = amount2 }
  void $ Emulator.waitNSlots 1
