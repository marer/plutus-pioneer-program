...
module Spec.HomeworkModelWithClose
    ( tests
    , test
    , TSModel (..)
    )  where
...
import           Week08.HomeworkTokenSaleWithClose           (TokenSale (..), TSStartSchema, TSUseSchema, startEndpoint, useEndpoints')

type TSUseSchema' = TSUseSchema .\/ Endpoint "init" TokenSale .\/ Endpoint "finalize" ()

useEndpoints'' :: Contract () TSUseSchema' Text ()
useEndpoints'' = awaitPromise $ endpoint @"init" useEndpoints' `select` finalize

finalize :: Promise () TSUseSchema' Text ()
finalize = endpoint @"finalize" $ \() -> useEndpoints''
...
tests :: TestTree
tests = testProperty "homework: token sale model with close" prop_TS
...
instance ContractModel TSModel where
            ...
            case m of
                Just t ->
                    do
                        let l = t ^. tssLovelace
                        let n = t ^. tssToken
                        deposit w $ lovelaceValueOf l <> assetClassValue (tokens Map.! w) n <> Ada.toValue minAdaTxOut
    ...
    perform h _ m (Close v w)        =
        do
            withWait m $ callEndpoint @"close" (h $ UseKey v w) ()
            when (w == v) $ forM_ wallets $ \w' ->
                    callEndpoint @"finalize" (h $ UseKey v w') ()
            delay 1
