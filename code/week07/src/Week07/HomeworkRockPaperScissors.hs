...
module Week07.HomeworkRockPaperScissors
    ( Game (..)
    , GameChoice (..)
    , FirstParams (..)
    , SecondParams (..)
...
check bsRock' bsPaper' bsScissors' (GameDatum bs (Just c2)) (Reveal nonce c1) _ =
    sha2_256 (nonce `appendByteString` choiceToBs c1) == bs
...
choiceToBs :: GameChoice -> BuiltinByteString
...
firstGame fp = do
    ...
    let game   = Game
        ...
        c      = fpChoice fp
        bs     = sha2_256 $ fpNonce fp `appendByteString` choiceToBs c
    ...
    case m of
        ...
        Just (o, _) -> case tyTxOutData $ ocsTxOut o of
            ...
            GameDatum _ (Just c') | c == c' -> do
                logInfo @String "second player played and there was a draw"
                void $ mapError' $ runStep client $ Reveal (fpNonce fp) c
                logInfo @String "first player revealed a draw"
